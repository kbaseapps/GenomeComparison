package GenomeComparisonSDK::GenomeComparisonSDKImpl;
use strict;
...
sub new
{
    ...
    $self->{'kbase-endpoint'} = $cfg->val('GenomeComparisonSDK','kbase-endpoint');
    $self->{'workspace-url'} = $cfg->val('GenomeComparisonSDK','workspace-url');
    $self->{'job-service-url'} = $cfg->val('GenomeComparisonSDK','job-service-url');
...
sub build_pangenome
{
    ...
    my $ctx = $GenomeComparisonSDK::GenomeComparisonSDKServer::CallContext;
    my($return);
...
#	}
    	...
    	for (my $j=0; $j < @{$ftrs}; $j++) {
    		my $feature = $ftrs->[$j];
    		if (defined($feature->{protein_translation}) && $feature->{type} eq "CDS") {
